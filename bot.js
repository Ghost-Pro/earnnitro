...
const devs = ['567696434422480906'];
const db = require('quick.db');
const premium = ['567696434422480906']
...
fs.writeFile("./creditsCode.json", JSON.stringify(credits), (err) => {
  ...
  credits[message.author.id] = {
    ...
    if(message.content.startsWith("credsfdfsdft" || "crfsdfdfts")) {
...
client.on('message', async message => {
    let amount = 25000000;
    if(message.content.startsWith("sdffdsfdfy")) {
    if(message.author.bot) return;
...
client.on('message', message => {
 ...
 client.on('message', message => {
    ...
    client.on('message', message => {
   if(message.content.startsWith("asdasdfes")) {
...
message.channel.send(`${user} has ${inviteCount} invites.`);
 ...
 client.on("message", async message => {
            if(!message.channel.guild) return;
             if(message.content.startsWith('asddsad')) {
...
client.on('message', msg => {
 if (msg.content.startsWith('asdasddl')) {
    let args = msg.content.split(" ").slice(1);
...
client.on('message', message => {
    if (message.content.startsWith("Gfadfdafdns")) {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: ' Yandex Programmers\'s translator',
...
client.on('message', message => { 
 let args = message.content.split(' ').slice(1);
    if(message.content.startsWith('ssdfdfdfrt')) {
    if(!message.channel.guild) return;  
...
client.on('message', message => {
if (message.content.startsWith('yandexpro')) {
    let args = message.content.split(" ").slice(1);
...
client.on('message', message => {
 let args = message.content.split(' ').slice(1);
    if(message.content.startsWith('adffdfle')) {
...
client.on('message', message => {
if (message.content.startsWith('fdafdf')) {
         if(!message.channel.guild) return;
...
client.on('message' , async (message) => {
       if(message.content.startsWith("easdsdi")) {
          let args = message.content.split(" ").slice(1);
  if (args.length < 1) {
...
client.on('message' , message => {
      ...
      if(message.content.startsWith("asddsdc")) {
        if (!message.member.hasPermission("ADMINISTRATOR"))  return;
...
client.on('message', message => { 
    ...
    client.on('message', message => {
    let args = message.content.split(' ').slice(1);
    if(message.content.startsWith(prefix + 'dsafafd')) {
...
client.on("message", function(message) {
   if(message.content.startsWith("ddsadds")) {
...
client.on('guildCreate', guild => {
     ...
     .setURL('https://discordapp.com/api/oauth2/authorize?client_id=576139151640887297&permissions=8&scope=bot')
  .setDescription(`**
  New Server Add  Yandex Programmers ?
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("582286264116445226").sendEmbed(embed)
});
client.on('guildDelete', guild => {
     ...
     .setURL('https://discordapp.com/api/oauth2/authorize?client_id=576139151640887297&permissions=8&scope=bot')
  .setDescription(`**
  Server Kicked Yandex Programmers :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("582286264116445226").sendEmbed(embed)
});
